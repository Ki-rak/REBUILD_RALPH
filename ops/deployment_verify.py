...
def main():
    ...
    parser.add_argument('--environment',default='deployed',choices=['deployed'])
    ...
    report={'product':'RE:Build Agent','boundary':'REAL_VERCEL_HTTPS_SUPABASE_OPENAI','environment':args.environment,'run_id':run,'at':datetime.now(timezone.utc).isoformat(),'checks':[],'product_complete':False,'deployment':True,'url':'https://rebuild-ralph.vercel.app','reviewer':'AUTOMATED_DISPOSABLE_TEST_ACCOUNT_ONLY','status':'PENDING','cleanup_complete':True}
    with httpx.Client(timeout=20,trust_env=False) as admin:
        try:
            s.validate();require(bool(key),'SUPABASE_SECRET_KEY_REQUIRED')
            stage='public_deployment_health'
            with httpx.Client(timeout=60,trust_env=False) as public:
                health=public.get(report['url']+'/api/health');require(health.status_code==200 and health.json().get('environment')=='deployed','DEPLOYED_HEALTH_REQUIRED')
                for asset in ['app.js','styles.css']:
                    reply=public.get(report['url']+'/'+asset);require(reply.status_code==200 and reply.content==(ROOT/'frontend'/asset).read_bytes(),'DEPLOYED_ASSET_MISMATCH')
            check(report,'public_health_and_exact_latest_ui_assets')
            ...
            with httpx.Client(base_url=report['url'],headers={'Origin':report['url']},timeout=280,trust_env=False,follow_redirects=False) as client:
                ...
                for kind in ['risk','slides']:
                    stage='actual_'+kind+'_approval_export'
                    extra=api.json('POST',f'/api/projects/{pid}/drafts',json={'kind':kind,'mode':'rules'})
                    require(extra.get('rows'),'OUTPUT_ROWS_REQUIRED')
                    api.json('POST',f"/api/drafts/{extra['id']}/approve",json={'confirmed':True,'revision':extra['revision']})
                    content=api.request('POST',f"/api/drafts/{extra['id']}/export").content
                    structure=office_signature(content,kind)
                    suffix='pptx' if kind=='slides' else 'xlsx';(target/f'{kind}.{suffix}').write_bytes(content)
                    check(report,'actual_'+kind+'_download_reopened',bytes=len(content),**structure)
                api.logout()
            stage='new_https_session'
            with httpx.Client(base_url=report['url'],headers={'Origin':report['url']},timeout=280,trust_env=False,follow_redirects=False) as client:
                ...
                api.logout()
            check(report,'new_https_session_and_login_preserve_actual_ai_usage_and_approval',process_restart=False)
    ...
    path=write_report(report,'vercel-live-verification');print(json.dumps({'status':report['status'],'environment':args.environment,'stage':report.get('stage'),'cleanup_complete':report['cleanup_complete'],'report':str(path.relative_to(ROOT))}))
